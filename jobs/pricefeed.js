...
async function getLatestDataToSign(metadata, configData) {
  try {
    ...
    let idx = configData[chainId].contracts.findIndex(
      (e) => e.toLowerCase() == contractAddress.toLowerCase(),
    )
...
async function validateOracleData(data, metadata, configData) {
  ...
  if (
    ct.options.address.toLowerCase() != metadata.contractAddress.toLowerCase()
  ) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (diffInPercentX10 > threshold) {
      logger.warn(
        'Token price differ over threshold, token %s price received %d, price local %d',
        tokens[i],
        decoded.prices[i],
        price,
      )
...
function signersSufficient(metadata, configData, currentNumSigners) {
  let oracleAddresses =
    configData[metadata.chainId].oracleAddresses[metadata.contractAddress]
  ...
  for (var i = 0; i < chainIdList.length; i++) {
    ...
    if (!priceFeedInfoMap[chainId]) {
      try {
        let rpc = config.rpc[chainId].http
        let web3 = new Web3(
          new HDWalletProvider(process.env.SUBMITTER_KEY, rpc),
        )
        ...
        for (var j = 0; j < contractMap[chainId].length; j++) {
          ...
          let ct = await new web3.eth.Contract(
            MultiPriceFeedABI,
            contractMap[chainId][j],
          )
          priceFeedInfoMap[chainId].contracts.push(ct.options.address)
          priceFeedInfoMap[chainId].tokens.push(
            await ct.methods.getTokenList().call(),
          )
          priceFeedInfoMap[chainId].descriptions.push(
            await ct.methods.description().call(),
          )
          let oracleAddresses = await ct.methods.getOracles().call()
          priceFeedInfoMap[chainId].oracleAddresses[
            ct.options.address
...
async function submitTransaction(metadata, configData, oracleData, r, s, v) {
  ...
  try {
    ...
    if (currentRound != parseInt(decoded.roundId)) {
      ...
      await ct.methods
        .submit(decoded.roundId, decoded.prices, decoded.deadline, r, s, v)
        .estimateGas({ from: configData.ACCOUNT, gas: 2000000 })
      var gp = config.gasPrices[chainId]
      console.log('submitting with gas price', gp)
      await ct.methods
        .submit(decoded.roundId, decoded.prices, decoded.deadline, r, s, v)
        .send({ from: configData.ACCOUNT, gas: 2000000, gasPrice: gp })
    }
  } catch (e) {
